...
### Privacy
- Uninstall feedback opens without automatically attaching extension details.
...
### Fixed
- Back to Main is now available to Free and Pro users at the top of Options, including mobile browsers.
...
## [5.1.23] - 2026-08-25
### Security
- Browser DNR rules, matching existing-tab cleanup, Daily Limit tracking, and Whitelist Focus now protect the exact Google authentication, YouTube account-service, project, and browser-store domains without trusting lookalike hosts or URL text.
...
- Store-domain exclusions remain target-specific, preserving existing Edge protections for Bing and Microsoft without applying those restrictions to Chrome.
- Existing schedules, active-profile isolation, Focus authorization, genuine legacy access, Free deletion, the exact ten-rule limit, and workers without the optional windows API remain unchanged.

### Added
- Added 20 regression scenarios for precise authentication and project domains, store-specific exclusions, partial-pattern preservation, direct host validation, spoofed lookalikes, custom redirects, DNR signatures and recovery, Daily Limits, existing tabs, Whitelist Focus, and windowless service workers.
...
- Password-protected logout, genuine legacy access, Free rule deletion, the exact ten-rule limit, Focus/DNR recovery, and the unchanged one-minute watchdog remain intact.

### Added
- Added 40 regression scenarios for forged runtime Pro and legacy claims, private credential responses, worker-owned activation, strict server approval, candidate validation, HTTP outcomes, retryable failures, timeout cleanup, storage failures, serialized activation/logout races, and windowless browser workers.
...
- Startup and the unchanged one-minute watchdog restore missed overnight activation, close matching open tabs, and remove expired browser blocking after delayed mobile wake-ups without redundant DNR writes or repeated tab scans across midnight.
...
- Free deletion, inherited-schedule cleanup, the exact ten-rule limit, active-profile isolation, disabled categories, Focus behavior, and windowless mobile worker compatibility remain unchanged.

### Added
- Added 55 regression scenarios covering overnight validation and activation, weekday rollovers, exact interval boundaries, mixed periods, schedule UI/presets/formatting, Rule Packs/imports, browser DNR recovery, delayed and windowless mobile workers, paid authorization, former-Pro cleanup, and stable Daily Limit journal fixtures.

## [5.1.19] - 2026-08-22
### Fixed
- Focus Sessions whose one-shot completion alarm was missed during sleep, shutdown, or a delayed mobile worker wake-up are now completed from persisted state during startup or the unchanged one-minute watchdog.
...
- Recovery remains available to Free and former-Pro users, Free rule deletion and the exact ten-rule limit remain unchanged, and mobile workers still do not require the optional windows API.

### Added
- Added 26 regression scenarios for missed and stale alarms, startup and minute recovery, callback alarm APIs, exact-boundary expiry, concurrent Focus transitions, failed storage and side effects, DNR repair, Free and legacy access, and windowless mobile workers.
...
- Chromium, Edge, and mobile browsers remain compatible with missing optional DNR limit constants without requiring the optional Windows API.
...
### Added
- Added write-budget, batch-remapping, quota-failure, permission watchdog, telemetry-cap, and windowless-mobile-worker regression coverage.
...
- Blocking and Whitelist Focus cleanup now preserve every affected desktop browser window independently instead of protecting only the final tab across all windows.
- Browsers without the optional Windows API use an unscoped safety tab, preserving mobile compatibility without accessing `chrome.windows`.
...
- Added tab-cleanup coverage for per-window safety, superseded Whitelist scans, and windowless mobile fallback behavior.
...
- Protected project, Chrome Web Store, and Microsoft Edge resources are identified by their actual hostnames, preventing unrelated sites from escaping Whitelist Focus through crafted paths, queries, fragments, or lookalike domains.
...
- Chromium and Edge background startup no longer assumes the optional `chrome.windows` API exists, preserving Daily Limit window-focus handling when the browser provides it.
...
### Added
- Added regression coverage for Pro-to-Free recovery, preserved Rule Lists, quota enforcement, inherited whitelist entries, Options and Popup entry points, and DNR integrity.
...
- Added service-worker coverage for deleting existing General rules and removing leftover custom Rule List assignments after Pro access is lost.
...
## [4.8.3] - 2026-08-11
### Improved
- Extended opt-in technical telemetry with aggregated counters for feedback prompt displays, review clicks, support clicks, and dismissals.
- Routed popup feedback counters through the service worker and the existing strict telemetry allowlist, while keeping telemetry disabled by default and schema version 1 unchanged.
- Added regression coverage for feedback counter reporting, allowlist rejection, Chromium callback error handling, and service-worker aggregation.
...
- Added privacy-safe error fingerprints for DNR, permission, license, rule, Focus Session, service worker, popup, and Options failures.
...
## [4.0.0] – 2026-07-14
### 🚨 BREAKING CHANGES
- **Storage Migration:** Rules database has been migrated from `chrome.storage.sync` to `chrome.storage.local`. Automatic background synchronization of rules across different devices is no longer supported. General extension settings (theme, toggles) are still synchronized via sync storage.
### Added
- Unlimited capacity for power users (up to 10MB+ storage via `local` API), completely bypassing Chrome's legacy `QUOTA_BYTES_PER_ITEM` errors.
...
## [3.10.8] – 2026-07-09
### Improved
- Improved support for Microsoft Edge users.
- Updated Edge store link handling and added protection for Bing-related URLs.
...
## [3.10.1] - 2026-06-30
### Fixed
- **Options Page:** Added real-time reactive UI locking/unlocking for rule management. The settings page now instantly reflects focus session status changes (started or stopped via popup) using `chrome.storage.onChanged`.
...
### Improved
- Updated protected patterns to prevent blocking essential Microsoft-related URLs.

## [3.9.9] - 2026-06-23
### Improved
- Added a link to the Edge Add-ons store in the feedback section.
...
## [3.7.3] — 2026-04-26
### Patch
- Added four new localization keys across all supported languages.
...
## [3.7.1] — 2026-04-23
### Fixed
- Fixed an issue with blocking rule ID generation.
...
## [2.16.19] - 2025-11-01
### Fixed
- Fixed an incorrect function call in the update check logic.
