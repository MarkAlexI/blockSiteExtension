...
The server accepts schema v1 requests from already released extension versions. Schema v1 batches have no `deliveryId`, so exact retry deduplication is not possible for those legacy deliveries. New Chromium clients starting with 4.8.4 send schema v2.
...
- allow only known context enum values;
