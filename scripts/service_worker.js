...
import { getTelemetryConsent } from '../telemetry/telemetryConsent.js';
...
function getFocusCompletionAlarm() {
  let result;
  try {
    result = chrome.alarms.get(FOCUS_COMPLETION_ALARM);
  } catch (_) {
    result = undefined;
  }
  ...
  return new Promise((resolve, reject) => {
    try {
      chrome.alarms.get(FOCUS_COMPLETION_ALARM, alarm => {
...
async function ensureFocusCompletionAlarm(endTime, shouldContinue) {
  ...
  await chrome.alarms.create(FOCUS_COMPLETION_ALARM, { when: endTime });
  return true;
}
...
async function showFocusCompletionNotification() {
  ...
  chrome.notifications.create('focus_session_ended', {
    type: 'basic',
...
async function reconcileStoredFocusSession(reason, {
  ...
  await chrome.storage.local.set({ focusSession: { ...INACTIVE_FOCUS_SESSION } });
  ...
  await Promise.all([
    runFocusCompletionTask(
      ...
      () => chrome.alarms.clear(FOCUS_COMPLETION_ALARM)
    ),
    runFocusCompletionTask(
    ...
    (async () => {
      try {
        const completed = await enqueueFocusSessionTransition(async () => {
          ...
          await chrome.storage.local.set({ focusSession: nextFocusSession });
          if (transitionGeneration !== focusSessionTransitionGeneration) return false;

          await chrome.alarms.create('end_focus_session', { when: endTime });
          ...
          if (syncResult?.success === false) {
            await chrome.storage.local.set({ focusSession: previousFocusSession });
            if (transitionGeneration !== focusSessionTransitionGeneration) return false;
            ...
            if (previousFocusAlarm) {
              ...
              if (Number.isFinite(previousEndTime) && previousEndTime > 0) {
                await chrome.alarms.create('end_focus_session', { when: previousEndTime });
              }
            } else {
