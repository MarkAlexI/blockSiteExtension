...
(async () => {
  ...
  chrome.storage.sync.get('rules', async ({ rules }) => {
    rules = rules || [];
    ...
    chrome.storage.sync.get('rules', async ({ rules }) => {
      ...
      if (ruleExists) {
        ...
        try {
          ...
          const urlFilter = `||${filter}`;
          const action = redirectURL.value.trim() ? { type: "redirect", redirect: { url: redirectURL.value.trim() } } : { type: "redirect", redirect: { url: chrome.runtime.getURL("blocked.html") } };
