...
function withChrome(run) {
  ...
  return Promise.resolve(run()).finally(() => { globalThis.chrome = previous; });
}

test('telemetry UI loads opt-in state and updates explicit consent', async () => withChrome(async () => {
...
test('telemetry UI restores the previous choice when consent update fails', async () => withChrome(async () => {
  const checkbox = new FakeElement();
  const status = new FakeElement();
  const ui = new TelemetryUI({
    checkbox,
