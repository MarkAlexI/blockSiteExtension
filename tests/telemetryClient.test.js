  ...
  const store = createStore([
    {
      date: '2026-08-09',
      context: {
        extensionVersion: '4.8.0', browser: 'chrome', browserMajor: 140,
    ...
    },
    {
      date: '2026-08-10',
      context: {
        extensionVersion: '4.8.1', browser: 'chrome', browserMajor: 140,
...
test('telemetry delivery failure schedules a real retry and success cancels it', async () => {
  ...
  const client = createTelemetryClient({
    localStorage: storage,
    store,
    getContext: async () => ({ extensionVersion: '4.8.1', browser: 'chrome' }),
    now: () => now,
...
test('telemetry client retries the same prepared delivery ID after a failed request', async () => {
  ...
  const client = createTelemetryClient({
    localStorage: storage,
    store,
    getContext: async () => ({ extensionVersion: '4.8.4', browser: 'chrome' }),
    now: () => now,
    fetchFn: async (_url, options) => {
