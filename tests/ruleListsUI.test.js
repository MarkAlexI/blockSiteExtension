...
test('rule list grid renders profile counts and marks exactly one active profile', async () => {
  ...
  globalThis.chrome = { i18n: { getMessage(key) { return key === 'rulelist_general' ? 'General' : key; } } };
...
test('resolveRuleListContext uses only known profiles', async () => {
  const previousApi = globalThis.chrome;
  globalThis.chrome = { i18n: { getMessage(key) { return key; } } };
  try {
