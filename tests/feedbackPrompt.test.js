...
test('review and support actions complete the prompt and open the correct destination', async () => {
  const localStorage = createStorage();
  const syncStorage = createStorage();
  const opened = [];
  const tabsApi = {
  ...
  const controller = createFeedbackPromptController({
    ...
    syncStorage,
    tabsApi,
    getStore: () => ({ reviewUrl: 'https://store.example/reviews' }),
  ...
  assert.deepEqual(opened, ['https://store.example/reviews']);
...
function createFeedbackDocument() {
  ...
  const elements = {
    ...
    'feedback-store-label': new Element()
  };
...
test('feedback prompt reports display and review click counters at the actual UI actions', async () => {
  ...
  const shown = await initFeedbackPrompt({
    ...
    getStore: () => ({ name: 'Chrome Web Store', reviewUrl: 'https://store.example/reviews' }),
