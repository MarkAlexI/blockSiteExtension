...
test('uninstall URL never includes automatic extension or user context', async () => {
  const api = createExtensionApi({
    sync: { credentials: {
      isLegacyUser: true,
  ...
  await withExtensionEnvironment(api, async () => {
    ...
    assert.deepEqual(api.uninstallUrls, [
      'https://blockdistraction.com/uninstall.html'
    ]);
  });
});

test('uninstall URL does not depend on credentials or rules being available', async () => {
  const api = createExtensionApi();
  ...
  await withExtensionEnvironment(api, async () => {
    await updateUninstallURL();
    assert.equal(api.uninstallUrls[0], 'https://blockdistraction.com/uninstall.html');
  });
});
